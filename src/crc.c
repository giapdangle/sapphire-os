...
#ifndef BOOTLOADER
static uint16_t PROGMEM crctab[256] = {
#else
...
uint16_t crc_u16_block(uint8_t *ptr, uint16_t length){
	...
	for(uint16_t i = 0; i < length; i++){
		...
		crc ^= pgm_read_word(&crctab[temp]);
		
		ptr++;
	}*/
    
    crc = crc_u16_partial_block(0xffff, ptr, length);
...
uint16_t crc_u16_byte(uint16_t crc, uint8_t data){
	...
	#if !defined BOOTLOADER && !defined __SIM__
	crc ^= pgm_read_word(&crctab[temp]);
	#else
	crc ^= crctab[temp];
	#endif
...
/*
This code is saved in comments in case the inline algorithm is ever needed.
